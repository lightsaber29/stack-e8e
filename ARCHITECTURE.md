...
사용자가 대화창에 직접 입력한 내용, 이번에 직접 업로드/가져온 문서 원문, 그리고 사용자가
명시적으로 실행한 정리·분류 작업의 **대상 경험 하나**는 사용자 본인의 Claude 계정으로
전송될 수 있다. 그 밖의 저장된 Experience 원문, 검색 Query, Embedding 은 이 예외에 포함되지 않는다.
...
2. **전송 범위는 대화 입력으로 한정한다.** 사용자가 대화창에 직접 입력한 내용과
   ...
   사용자가 명시적으로 실행한 분류 채우기의 **대상 경험 하나**도 여기 포함된다 — 한 번의
   호출에 그 경험의 제목과 S/P/A/R 만 담고, 다른 Experience·검색 Query·Embedding 은 담지 않는다.
   사용자가 버튼/명령으로 시작하지 않으면 이 전송은 일어나지 않는다
   (2026-09-01 확장, 배경은 DECISIONS.md 참고).
...
5. **이 기능 없이도 앱은 동작해야 한다.** Claude Code 가 설치·로그인되어 있지 않으면
   대화형 입력과 분류 채우기만 비활성화되고, 폼 직접 입력 · 검색 · CRUD · 볼트 동기화 ·
   소재 문서 가져오기(분류 없이)는 그대로 오프라인으로 동작해야 한다.
